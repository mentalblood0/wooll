...
pub fn parse_commands_batch(
        ...
        .enumerate()
    {
        ...
        match lines.len() {
            0 => {
                return Err(anyhow!(
                    "{}-nth paragraph is empty, can not parse it as a command",
                    paragraph_index + 1
                ));
            }
            lines_count => {
                if let Some(captures) = command_first_line_regex.captures(lines[0]) {
                    ...
                    match (operation_char, lines_count) {
                        ('+', 2) => {
                            result.push(Command::AddTextThesis(AddTextThesis {
                                alias: alias_option,
                                text: Text(lines[1].to_string()),
                            }));
                        }
                        ('+', 4) => {
                            result.push(Command::AddRelationThesis(AddRelationThesis {
                                alias: alias_option,
                                from: (),
                                to: (),
                                kind: (),
                        ...
                        _ => {
                            return Err(anyhow!(
                                "Unsupported operation character and lines count combination ({:?}, {}) in the first line {:?} of {}-nth paragraph {:?}, supported combinations are ('+', 2) for adding text thesis, ('+', 4) for adding relation thesis, ('-', 2) for removing thesis, ('#', 3) for adding tag, ('^', 3) for removing tag",
                                operation_char,
                                lines_count,
                                lines[0],
                                paragraph_index + 1,
